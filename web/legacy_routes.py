...
from config.settings import Config # Assuming Config class handles read/write
...
config: Config = None # This is your Config object, which should handle saving
                ...
                if row[0]:  # extracted_data is not null
                    try:
                        ...
                        if isinstance(data, dict):
                            ...
                            if amount is not None and amount > 0: # Check for None explicitly
                                total_amount += amount
                                amount_count += 1
    ...
    try:
        ...
        return render_template('dashboard.html', 
                             ...
                             stats={
                                ...
                                'processing': 0,
                                'avg_amount': 0.0,
                                'total_amount': 0.0 # Added total_amount here for consistency
                             })
...
def configuration():
    ...
    global config # Ensure we modify the global config object
    ...
    if request.method == 'POST':
        try:
            ...
            # Only update API token if it's not empty (prevents accidental overwrites)
            ...
            try:
                ...
                config.set('bigcapital', 'default_due_days', str(default_due_days)) # Store as string if Config expects strings
            except ValueError:
                flash('Default Due Days must be a number.', 'warning')
                logger.warning('Invalid input for bigcapital_default_due_days')
                # Optionally, do not save and reload current config
            ...
            # Attempt to save changes to the configuration file
            # Assuming your Config class has a save() method
            config.save() 
            flash('Configuration saved successfully!', 'success')
            logger.info("Configuration updated and saved.")
            
            # Re-initialize main app components if configuration changes require it
            # This is critical for changes to take effect without restarting the app.
            # You might need to add a re-initialization function in your main app.py
            # For now, we'll just log and let the user know.
            current_app.config_reloaded = True # Custom flag to indicate reload needed
        ...
        return redirect(url_for('web.configuration')) # Redirect to prevent re-submission on refresh
        ...
        current_config_data = {
            'paperless_ngx': {
                ...
                'api_token': config.get('paperless_ngx', 'api_token', fallback=''), # Do NOT expose real token in value attribute on GET request for security. Send it empty.
            ...
            'bigcapital': {
                ...
                'api_token': '', # Same as above, do not expose token on GET
        ...
        return render_template('config.html', 
                               ...
                               log_levels=log_levels) # Pass log levels to template
...
def upload_file():
    ...
    try:
        ...
        upload_folder = config.get('processing', 'upload_folder', 'uploads') # Use config for upload_folder
        ...
        if not force_upload:
            duplicate_check = check_duplicate_file(original_filename, file_hash, file_info['file_size'])
        ...
        # Re-check in case another upload happened in between or something changed
        # This check might be redundant if the initial duplicate_check is reliable
        final_duplicate_check = check_duplicate_file(original_filename, file_hash, file_info['file_size'])
...
def check_duplicate():
    ...
    try:
        ...
        try:
            ...
            duplicate_check = check_duplicate_file(file.filename, file_hash, file_info['file_size'])
