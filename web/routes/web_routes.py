...
# Helper function to safely set configuration values, ensuring section exists
def set_config_value(config_obj, section, key, value):
    """
    Sets a configuration value in the config object.
    Ensures the section exists before attempting to set the key.
    Assumes config_obj behaves like configparser.ConfigParser.
    """
...
def create_web_blueprint(config, db_manager, doc_processor):
    ...
    def configuration():
        ...
        if request.method == 'POST':
            try:
                # Paperless-NGX
                # Replaced config.set with direct dictionary-like assignment
                # and used the helper function
                set_config_value(config, 'paperless', 'api_url', request.form.get('paperless_api_url'))
                if request.form.get('paperless_api_token'):
                    set_config_value(config, 'paperless', 'api_token', request.form.get('paperless_api_token'))
                set_config_value(config, 'paperless', 'invoice_tags', request.form.get('paperless_invoice_tags'))
                set_config_value(config, 'paperless', 'receipt_tags', request.form.get('paperless_receipt_tags'))

                # Bigcapital
                set_config_value(config, 'bigcapital', 'api_url', request.form.get('bigcapital_api_url'))
                if request.form.get('bigcapital_api_token'):
                    set_config_value(config, 'bigcapital', 'api_token', request.form.get('bigcapital_api_token'))
                
                try:
                    default_due_days = int(request.form.get('bigcapital_default_due_days'))
                    set_config_value(config, 'bigcapital', 'default_due_days', str(default_due_days))
                except ValueError:
                    flash('Default Due Days must be a number.', 'warning')
                    logger.warning('Invalid input for bigcapital_default_due_days')

                set_config_value(config, 'bigcapital', 'auto_create_customers', 'true' if 'bigcapital_auto_create_customers' in request.form else 'false')

                # Processing
                try:
                    check_interval_seconds = int(request.form.get('processing_check_interval_seconds'))
                    set_config_value(config, 'processing', 'check_interval_seconds', str(check_interval_seconds))
                except ValueError:
                    flash('Check Interval (seconds) must be a number.', 'warning')
                    logger.warning('Invalid input for processing_check_interval_seconds')

                set_config_value(config, 'processing', 'log_level', request.form.get('processing_log_level'))
                set_config_value(config, 'processing', 'processed_tag', request.form.get('processing_processed_tag'))
                set_config_value(config, 'processing', 'error_tag', request.form.get('processing_error_tag'))
