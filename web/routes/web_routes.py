...
def create_web_blueprint(config, db_manager, doc_processor):
            ...
            try:
                ...
                if "T" in value and ("Z" in value or "+" in value):
                    dt_obj = datetime.fromisoformat(
                        value.replace("Z", "+00:00") if "Z" in value else value
                    )
    ...
    def index():
        ...
        try:
            ...
            logger.info(
                f"Dashboard loaded with {len(recent_docs)} recent docs and stats: {stats}"
            )

            return render_template(
                "dashboard.html", recent_docs=recent_docs, stats=stats
            )
    ...
    def configuration():
        ...
        if request.method == "POST":
            try:
                ...
                # Paperless-NGX
                config.set(
                    "paperless", "api_url", request.form.get("paperless_api_url")
                )
                if request.form.get("paperless_api_token"):
                    config.set(
                        "paperless",
                        "api_token",
                        request.form.get("paperless_api_token"),
                    )
                config.set(
                    "paperless",
                    "invoice_tags",
                    request.form.get("paperless_invoice_tags"),
                )
                config.set(
                    "paperless",
                    "receipt_tags",
                    request.form.get("paperless_receipt_tags"),
                )

                # Bigcapital
                config.set(
                    "bigcapital", "api_url", request.form.get("bigcapital_api_url")
                )
                if request.form.get("bigcapital_api_token"):
                    config.set(
                        "bigcapital",
                        "api_token",
                        request.form.get("bigcapital_api_token"),
                    )

                try:
                    default_due_days = int(
                        request.form.get("bigcapital_default_due_days")
                    )
                ...
                config.set(
                    ...
                    "true"
                    if "bigcapital_auto_create_customers" in request.form
                    else "false",
                )
                ...
                try:
                    check_interval_seconds = int(
                        request.form.get("processing_check_interval_seconds")
                    )
                    config.set(
                        "processing",
                    ...
                    logger.warning(
                        "Invalid input for processing_check_interval_seconds"
                    )

                config.set(
                    "processing", "log_level", request.form.get("processing_log_level")
                )
                config.set(
                    "processing",
                    "processed_tag",
                    request.form.get("processing_processed_tag"),
                )
                config.set(
                    "processing", "error_tag", request.form.get("processing_error_tag")
                )
                ...
                logger.info(
                    "Application components may need re-initialization due to config changes."
                )
            ...
            current_config_data = {
                "paperless": {
                    ...
                    "invoice_tags": config.get(
                        "paperless", "invoice_tags", fallback=""
                    ),
                    "receipt_tags": config.get(
                        "paperless", "receipt_tags", fallback=""
                    ),
                },
                "bigcapital": {
                    "api_url": config.get("bigcapital", "api_url", fallback=""),
                    "api_token": "",  # Don't expose token on GET
                    "default_due_days": config.get(
                        "bigcapital", "default_due_days", fallback=""
                    ),
                    "auto_create_customers": config.getboolean(
                        "bigcapital", "auto_create_customers", fallback=False
                    ),
                },
                "processing": {
                    "check_interval_seconds": config.get(
                        "processing", "check_interval_seconds", fallback=""
                    ),
                    "log_level": config.get("processing", "log_level", fallback="INFO"),
                    "processed_tag": config.get(
                        "processing", "processed_tag", fallback=""
                    ),
            ...
            return render_template(
                "config.html", config=current_config_data, log_levels=log_levels
            )
