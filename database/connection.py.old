...
class DatabaseManager:
    ...
    def insert_document(self, filename: str, original_filename: str, 
                       file_path: str, file_size: int, content_type: str) -> int:
        """Insert a new document record"""
        query = '''
            INSERT INTO documents (filename, original_filename, file_path, file_size, content_type)
            VALUES (?, ?, ?, ?, ?)
        '''
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(query, (filename, original_filename, file_path, file_size, content_type))
            conn.commit()
            return cursor.lastrowid
    
    def update_document(self, doc_id: int, **kwargs) -> int:
    ...
    def get_documents(self, limit: int = 100, offset: int = 0) -> list:
